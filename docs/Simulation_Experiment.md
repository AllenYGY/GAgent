...
本说明覆盖 `scripts/simulation/parallel_simulation_experiment.py` 的两种模式：

- **plan**：对完整计划 JSON 做“模拟用户↔代理”循环评估
...
python scripts/simulation/parallel_simulation_experiment.py \
  --mode plan \
...
plan 模式的评估结果在 `eval/results.csv`，绘图应指向 **eval 目录**：
