...
- `/Users/allenygy/Research/GAgent/scripts/eval/eval_plan_quality.py`

该脚本使用 **0/1/2 标准**计算每个维度 **1–10 分**。维度包括：

- Contextual Completeness
- Accuracy
- Task Granularity & Atomicity
...
python scripts/eval/eval_plan_quality.py \
  --plan-tree-dir results/agent_plans_phage_gemini/plans \
  --provider openrouter \
  --model google/gemini-3-pro-preview \
  --batch-size 2 \
  --max-retries 3 \
  --output results/agent_plans_phage_gemini/eval/plan_scores.csv \
  --jsonl-output results/agent_plans_phage_gemini/eval/plan_scores.jsonl
```

注意：该评估需要真实 LLM API Key（非单元测试）。
