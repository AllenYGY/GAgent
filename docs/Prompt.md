...
## 3) plan 模式（直接生成完整 plan）

**位置**：`scripts/simulation/parallel_simulation_experiment.py`
...
plan 模式默认不保存 prompt，但会保存 raw/parsed 计划与 judge verdict。

## 4) Plan Quality 评估 Prompt

**位置**：`scripts/eval/eval_plan_quality.py`
